...
BACKUP_RSYNC_REMOTE=backup@backup.tupadr3.de:/data/backup/dev.tupadr3/borg
BACKUP_RSYNC_SSH="ssh -p 2605 -i /data/config/ssh/dev.tupadr3@backup.tupadr3.de"
BORG_PASSPHRASE='XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX'
...
function backupSetup()
{
	...
	mkdir $TEMP_DIR/temp -p 2>/dev/null
}

function backupFiles()
{
	log "Creating new gitlab backup..."
	docker exec -it gitlab sudo -HEu git bundle exec rake gitlab:backup:create >> $LOG_FILE
	
	log "Preliminary rsync..."
	rsync -vra --stats --exclude-from "exclude.txt" /data/docker/ --progress $TEMP_DIR/docker/ --delete >> $LOG_RSYNC_FILE
	rsync -vra --stats /data/logs/ --progress $TEMP_DIR/logs/ --delete  >> $LOG_RSYNC_FILE
...
# Transfer 
##########################
log "Transfering files to remote..."
metric "backup_time_start{type=\"rsync\"}" $(date +%s)
rsync -av --delete --progress -e "$BACKUP_RSYNC_SSH" -r -P $BORG_REPO/* $BACKUP_RSYNC_REMOTE >> $LOG_RSYNC_FILE
...
mv $LOG_FILE ${BORG_REPO}/backup.log

rm ${BACKUP_BASE}/rsync.log 2>/dev/null
mv $LOG_RSYNC_FILE ${BORG_REPO}/rsync.log

rm ${BACKUP_BASE}/backup.prom 2>/dev/null
mv $METRICS_FILE ${BORG_REPO}/backup.prom

rsync -av --delete --progress -e "$BACKUP_RSYNC_SSH" -r -P $BORG_REPO/* $BACKUP_RSYNC_REMOTE >> $LOG_FILE